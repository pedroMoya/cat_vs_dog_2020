...
def evaluate():
    ...
    try:
        ...
        try:
            ...
            column_names = ['id_number', 'class', 'filename', 'filepath']
            x_col = 'filepath'
            y_col = 'class'
            metadata_evaluation_images = \
                pd.read_csv(''.join([local_script_settings['models_evaluation_path'], 'evaluation_metadata.csv']),
                            dtype=str, names=column_names, header=None)
            metadata_evaluation_images['filepath'] = metadata_evaluation_images['filepath'].astype(str)
            test_set = test_datagen.flow_from_dataframe(dataframe=metadata_evaluation_images,
                                                        directory=None,
                                                        x_col=x_col,
                                                        y_col=y_col,
                                                        shuffle=False,
            ...
            print('\nadjusting evaluation to ~binary classification (if not already the case)')
